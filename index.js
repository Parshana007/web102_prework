...
const totalplegged = GAMES_JSON.reduce( (acc, game) => {return acc + game.pledged}, 0);
raisedCard.innerHTML = `<p>$${totalplegged.toLocaleString('en-US')}</p>`;
...
function filterUnfundedOnly() {
    ...
    let notmet = GAMES_JSON.filter(game => {return game.pledged < game.goal});
...
function filterFundedOnly() {
    ...
    let met = GAMES_JSON.filter(game => {return game.pledged >= game.goal});
...
// const totalfundedpledged = fundedGames.reduce( (acc, game) => {return acc + game.pledged}, 0); //finds total
// const totalunfundedpledged = unfundedGames.reduce( (acc, game) => {return acc + game.pledged}, 0);

// create a string that explains the number of unfunded games using the ternary operator
let display = `<p>A total of $${totalplegged.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length === 1 ? 'game' : 'games'}. 
Currently ${totalunfunded} ${totalunfunded == 0 ? 'games are funded': ` ${totalunfunded === 1 ? 'game' : 'games'} currently remain unfunded.`} 
${totalunfunded > 0 ? 'We need your help to fund these amazing games.' : 'Thanks for your continous support '}</p>`;
